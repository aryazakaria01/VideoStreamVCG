...
@Client.on_message(command(["vjoin", f"vjoin@{Cyber.BOT_USERNAME}"]) & ~filters.private & ~filters.bot)
...
@Client.on_message(command(["vleave", f"vleave@{Cyber.BOT_USERNAME}"]) & filters.group & ~filters.edited)
@authorized_users_only
...
@Client.on_message(command(["leaveall", f"leaveall@{Cyber.BOT_USERNAME}"]))
async def outall(client, message):
    if message.from_user.id not in Cyber.SUDO_USERS:
        return
